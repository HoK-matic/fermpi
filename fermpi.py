...
class FermentationThread(threading.Thread):
    ...
    def __init__(self, id, gpio):
        ...
        conn = mdb.connect('homenet', 'fermpi', 'frosttau97', 'fermpi')
        with conn as cur:
            i = 0
...
def read_configuration():
    ...
    with conn as cur:
        ...
        while row is not None:
            value = sql_number(row[1])
            if row[0] == 'state':
                if dictConfig['state'] != value:
                    dictConfig['state'] = value

                if dictConfig['state'] == FPI_STATE_OFF:
                    dictConfig['mode']= FPI_MODE_IDLE
                    dictConfig['cycle'] = int(10)
                    dictConfig['log'] = int(0)
                    break
            elif row[0] == 'mode':
                dictConfig['mode']= value
            elif row[0] == 'cycle':
                dictConfig['cycle'] = value
            elif row[0] == 'log':
                dictConfig['log'] = value
...
def main():
    ...
    logger.info(" Copyright (©) 2018 Holger Kupke")
            ...
            if dictConfig['state'] == FPI_STATE_OFF:
                if thread is None:
                    ...
                    while thread.is_alive():
                        thread.join(1.0)
                    thread = None
            elif dictConfig['state'] == FPI_STATE_ON:
                if thread is None:
                    logger.info(" ----------------------------------------")
                    logger.info(" State: ON")
                    if dictConfig['mode'] == FPI_MODE_IDLE:
                        thread = IdleMode(0, 0)
                        thread.start()
                    elif dictConfig['mode'] == FPI_MODE_CONSTANT:
                        thread = ConstantMode(dictConfig['log'], HEATER_GPIO)
                        thread.start()
                    elif dictConfig['mode'] == FPI_MODE_GRADUAL:
                        thread = GradualMode(dictConfig['log'], HEATER_GPIO)
                        thread.start()
            else:
                logger.warning(" Unknown controller state (%d)" % (dictConfig['state']))
